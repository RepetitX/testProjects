...
namespace Common.Web.Mvc.Controls
{
	...
	public class ColumnBuilder<T> : ICollection<GridColumn<T>> where T : class 
	{
		...
		/// </summary>
		/// <param name="propertySpecifier">Lambda that specifies the property for which a column should be constructed</param>
		/// <param name="name">Explicit name for the column. If name is not specified it cannot be made hidden and will be allways visible.</param>
		public IGridColumn<T> For(Expression<Func<T, object>> propertySpecifier, string name)
		{
			...
			var column = new GridColumn<T>(propertySpecifier.Compile(), name ?? inferredName, type);
			Add(column);
			return column;
		}
