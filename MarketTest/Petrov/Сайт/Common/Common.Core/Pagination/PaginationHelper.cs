...
namespace Common.Core.Pagination
{
	...
	public static class PaginationHelper
	{
        ...
        public static IPagination<T> AsCustomPagination<T>(this IEnumerable<T> source, int pageNumber, int pageSize, int totalItems)
        {
            if (pageNumber < 1)
            {
                throw new ArgumentOutOfRangeException("pageNumber", "The page number should be greater than or equal to 1.");
            ...
            return new CustomPagination<T>(source.AsQueryable(), pageNumber, pageSize, totalItems);
        }
	}
}
